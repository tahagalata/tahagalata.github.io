...
/**
 * "September 2024" -> "2024-09", for `datetime`. Returns undefined for values
 * that are not a month and year ("Present"), which get no <time> wrapper: a
 * bare <time> is only valid when its text is machine-readable on its own.
 */
...
/** A date that carries `datetime` when it can, and is plain text when it cannot. */
function RoleDate({ value }: { value: string }) {
  const iso = isoMonth(value);
  return iso ? <time dateTime={iso}>{value}</time> : <>{value}</>;
}
...
export function Experience() {
  ...
  return (
    <section aria-labelledby="experience-heading" className="mt-24 sm:mt-28">
      ...
      {/*
        The terminal edge: where the work is heading. It sits outside the <ol>
        so the list counts roles only — inside it, it was either a phantom fifth
        item or, when hidden from assistive tech, silently dropped the pivot the
        whole graph exists to show.
      */}
      <div className="mt-8 mb-4 grid grid-cols-[24px_1fr] gap-x-4 sm:gap-x-6">
              ...
              <article className="rounded-[2px] border border-hairline bg-surface p-5 sm:p-6">
                ...
                <header className="sm:flex sm:items-baseline sm:justify-between sm:gap-6">
                  ...
                  <p className="mono mt-2 shrink-0 text-xs text-muted sm:mt-0">
                    <RoleDate value={role.start} />
                    <span className="mx-1.5 text-edge">–</span>
                    <RoleDate value={role.end} />
                  </p>
                </header>
