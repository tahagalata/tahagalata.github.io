...
- `public/og-v3.png`, **1200x630 native** (source: `scratchpad/og/index.html`, rendered with chrome-headless-shell at `--force-device-scale-factor=1`). Not the portrait: at 431x442 it cannot fill that frame sharply.
- **Render at exactly 1200x630, never 2x.** A 2400x1260 card shipped first and LinkedIn rendered it badly blurred: LinkedIn resamples anything off-size with a poor resizer, and text was the first thing to dissolve. Shipping at native size means no resampling in the chain. A local downscale test is *not* evidence the card survives — sips/Lanczos kept it crisp at 640px while LinkedIn still destroyed it.
- Social platforms cache scraped previews hard, so the card must exist before the URL is shared anywhere.
- Changing the filename (`og.png` → `og-v2` → `og-v3`) forces LinkedIn to fetch fresh. It did *not* fix the blur, so it is a cache-bust tool, not a quality fix — bump the suffix whenever the card is re-cut, but don't mistake it for a diagnosis.
